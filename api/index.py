...
def sb_get(table, filters=None, order=None, limit=500):
    ...
    r = httpx.get(f"{SUPABASE_URL}/rest/v1/{table}", headers=SB_HEADERS, params=params, timeout=10)
    r.raise_for_status()
    return r.json()


def sb_insert(table, row):
    r = httpx.post(f"{SUPABASE_URL}/rest/v1/{table}", headers=SB_HEADERS, json=row, timeout=10)
    r.raise_for_status()
    return r.json()
...
def _td(label, value):
    ...
    return (
        f"<tr>"
        f"<td style='padding:8px 12px;border-bottom:1px solid #E2E8F0;color:#64748B;font-size:12px;"
        f"text-transform:uppercase;letter-spacing:1px;white-space:nowrap;width:38%'>{label}</td>"
